...
isdir(pwd() * "\\Figs_ExperimentalData\\") || mkdir(pwd() * "\\Figs_ExperimentalData") # make folder to save figures if doesnt already exist
filepath_save = [pwd() * "\\Figs_ExperimentalData\\"] # location to save figures "\\Fig2\\"] # location to save figures
...
data_totallivingcells = [9.128543425;9.748838625; 8.480052989; 5.716919827; 6.478056945; 4.955514183; 2.699895275;3.094628584;2.248771493  ] .*10^4; 
...
Random.seed!(1234)
   ...
   if fmle_tmp > fmle;
    ...
    for i in 1:(nptss)
        ...
        plot!(fig_data_totals_mle_update,t_smooth,totallivingcells_true_tsmooth,lw=2,color=:gray,ls=:dash) # true model solution
        ...
        profile1=vline!([ β1 ],lw=3,linecolor=:gray,linestyle=:dash)
        ...
        profile2=vline!([NN1],lw=3,linecolor=:gray,linestyle=:dash)
...
data0_structured = hcat(C0, C24, C48);
...
for k=1:length(t_structured)
    ...
    # scatter!(fig_logproportion,([0:(n_max);]),log10.(data_tmp),msw=0,ylim=(-5,0),color=:black);
    scatter!(fig_logproportion,([0:(n_max);]),log10.(data_tmp),markersize=2,msw=0,ylim=(-5,0),color=:black,ls=:dot,lw=2);
